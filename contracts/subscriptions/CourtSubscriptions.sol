...
contract CourtSubscriptions is ControlledRecoverable, TimeHelpers, ISubscriptions {
    ...
    event FeesPaid(address indexed subscriber, ERC20 feeToken, uint256 feeAmount, bytes ref);
    event FeesDonated(address indexed payer, ERC20 feeToken, uint256 feeAmount);
    ...
    */
    function donate(uint256 _amount) external payable {
        ...
        emit FeesDonated(msg.sender, feeToken, _amount);
    }
    ...
    */
    function _payFees(address _from, address _to, bytes memory _reference) internal {
        ...
        emit FeesPaid(_to, feeToken, feeAmount, _reference);
    }
